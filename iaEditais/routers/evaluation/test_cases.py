...
async def create_test_case(
    ...
    test_collection = await evaluation_repository.get_test_collection(session, test_case.test_collection_id)
...
async def update_test_case(
    ...
    if test_case_data.name:
        db_test_case = await evaluation_repository.get_test_case(session, test_case_id)
        if not db_test_case:
            raise HTTPException(
                status_code=HTTPStatus.NOT_FOUND,
                detail='Test case not found',
    ...
    updated_test_case = await evaluation_repository.update_test_case(
        session, test_case_id, test_case_data.model_dump(exclude_unset=True), current_user
    )
