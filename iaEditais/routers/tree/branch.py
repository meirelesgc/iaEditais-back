...
from iaEditais.models import branch_model
...
@router.post(
    '/taxonomy/branch/',
    response_model=branch_model.Branch,
    status_code=HTTPStatus.CREATED,
)
async def post_branch(
    branch: branch_model.CreateBranch, conn: Connection = Depends(get_conn)
):
    return await taxonomy_service.post_branch(conn, branch)


@router.get(
    '/taxonomy/branch/{taxonomy_id}/', response_model=list[branch_model.Branch]
)
